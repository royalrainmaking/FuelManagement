...
// ฟังก์ชันสร้าง UID 10 ตัว เช่น "FT1805K2X9"
//   FT = prefix (2)
//   DD = วันที่ (2)
//   MM = เดือน (2)
//   4 chars = สุ่ม alphanumeric (4)
function generateUID() {
    ...
    let randomPart = '';
    for (let i = 0; i < 4; i++) {
        randomPart += alpha[Math.floor(Math.random() * alpha.length)];
    }

    const newUID = `FT${dd}${mo}${randomPart}`; // เช่น FT0905K2X9 (10 ตัว)
